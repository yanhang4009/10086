...
def convert_to_hierarchical_list(input_str):
    levels = input_str.split('--')
    result = ""

    for index, level in enumerate(levels):
        result += f"{'-' * (index + 1)}{level}\n"

    return result

def get_input_texts(table_name):
    lines = []
    lines.append({"doc_label": [], "doc_token": list(jieba.cut(table_name)), "doc_keyword": [], "doc_topic": []})
    lines.append({"doc_label": [], "doc_token": ["海阳市", "纳入", "烟台", "倍增", "计划", "企业名单"], "doc_keyword": [], "doc_topic": []})
    return lines

def run(input_file):
    argv1, argv2 = "htc/conf/train.hierar.json", "htc/data/predict.json"
    config = Config(config_file=argv1)
    predictor = Predictor(config)
    batch_size = config.eval.batch_size
    input_texts = []
    predict_probs = []
    is_multi = config.task_info.label_type == ClassificationType.MULTI_LABEL
    for line in codecs.open(argv2, "r", predictor.dataset.CHARSET):
        input_texts.append(line.strip("\n"))
    print(input_texts)
    input_texts = get_input_texts(input_file.split(".")[0])
    print(input_texts)
    epoches = math.ceil(len(input_texts)/batch_size)

    predict_label_name_list = []
    ...
    for predict_prob in predict_probs:
        ...
        predict_label_name_list.append(predict_label_name)
    return convert_to_hierarchical_list(predict_label_name_list[0][0])
